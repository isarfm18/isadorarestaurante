...
2. /views/: Camada de interface (frontend), utilizando EJS para renderização dinâmica das páginas do sistema.

3. /config/: Responsável pelas configurações do ambiente e conexão com o banco de dados.

4. index.js: O ponto de entrada da aplicação, onde configuramos o Express e as rotas da API.

5. init.sql: Script de inicialização do banco de dados para garantir a consistência dos dados em qualquer ambiente.

6. sonar-project.properties: Arquivo de configuração que guia o SonarQube na análise de qualidade do nosso código.
